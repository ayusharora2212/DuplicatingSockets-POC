...
func main() {
	...
	if err != nil {
		fmt.Println("Error in creating named pipe:", err)
		return
	}
	...
	var receivedData azcri.WSADuplicateSocketInfo
	err = proto.Unmarshal(buf[:bytesRead], &receivedData)
	if err != nil {
		fmt.Println("Error unmarshalling protobuf data:", err)
		return
	}

	fmt.Println("Received duplicate socket info:", receivedData)

	reader := bytes.NewReader(receivedData.ProtocolInfo)
	err = binary.Read(reader, binary.LittleEndian, &info)
	if err != nil {
		fmt.Println("Error deserializing WSAProtocolInfo:", err)
		return
	}
	...
	if err != nil {
		fmt.Println("Error creating new scoket:", err)
		return
	}
...
func handleConnection(sock windows.Handle) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				fmt.Println("Error reading data from socket:", err)
				return
			}
			if bytesRead > 0 {
